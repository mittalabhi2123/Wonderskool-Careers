...
public class ProcessSingleResult extends JFrame
{
  ...
  private void initComponents()
  {
    ...
    layout.setHorizontalGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(layout.createSequentialGroup().addContainerGap().addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.jScrollPane2, -2, 430, -2).addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(this.errMsgLbl, -1, 547, 32767).addGroup(layout.createSequentialGroup().addComponent(this.enrollLbl).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.enrollTxt, -2, 111, -2)).addGroup(layout.createSequentialGroup().addComponent(this.saveBtn).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.resultBtn).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.showBtn, -2, 167, -2).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.cancelBtn)))).addContainerGap(-1, 32767)));

    layout.setVerticalGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(layout.createSequentialGroup().addContainerGap().addComponent(this.errMsgLbl).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addGroup(layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(this.enrollLbl).addComponent(this.enrollTxt, -2, -1, -2)).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.jScrollPane2, -1, 217, 32767).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addGroup(layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(this.saveBtn).addComponent(this.cancelBtn).addComponent(this.resultBtn).addComponent(this.showBtn)).addContainerGap()));
  ...
  private void saveBtnActionPerformed(ActionEvent evt) {
    ...
    try {
      ...
      ResultSet dataSet = Utility.conn.createStatement().executeQuery(QueryCollection.STUDENT_RESULT.GET_EXISTING_DATA.replace("?", enrollNo.toLowerCase()));
      while (dataSet.next()) {
        marksMap.put(dataSet.getString(4), Double.valueOf(dataSet.getDouble(5)));
      }
      if (marksMap.isEmpty()) {
        this.errMsgLbl.setText("Error:No Result Generated!!!");
        return;
      }
  ...
  private void showBtnActionPerformed(ActionEvent evt) {
    ...
    try {
      ...
      for (String key : TraitVariablesLst.dataMap.keySet()) {
        str.append(new StringBuilder().append("<html> <u><i><b><font size=8>").append(key).append("</font></b></i></u><br>").toString());
  ...
  public static boolean loadCutOffData(LinkedHashMap<String, Integer> cutOffMap)
    throws Exception
  {
  ...
  public static boolean loadProfessionData(HashMap<String, List<String>> prof2CriteriaMap, HashMap<String, List<String>> criteria2ProfMap) throws Exception {
    ResultSet dataSet = Utility.conn.createStatement().executeQuery("SELECT profession,criteria FROM profession_criteria");
    boolean dataExists = false;
    String profName = "";
    String criteria = "";
    while (dataSet.next()) {
  ...
  public static void addData2DB(String instName, String enrollNum, String profTrait, String traitVar, String profession) throws Exception {
    Utility.conn.createStatement().executeUpdate(QueryCollection.STUDENT_PROFESSION.DELETE_EXISTING_DATA.replace("?", enrollNum.toLowerCase()).replace("~", profTrait.toLowerCase()));
    PreparedStatement insertData = Utility.conn.prepareStatement("INSERT INTO student_profession VALUES (?, ?, ?, ?, ?)");
  ...
  public static void processMarksData(String instName, String enrollNo, HashMap<String, Double> marksMap, LinkedHashMap<String, Integer> cutOffMap, HashMap<String, List<String>> criteria2ProfMap, HashMap<String, List<String>> prof2CriteriaMap) throws Exception {
    ...
    for (String key : TraitVariablesLst.dataMap.keySet()) {
      ...
      for (String criteria : TraitVariablesLst.dataMap.get(key)) {
        ...
        if (((key.equalsIgnoreCase("Quality Graph")) || (marks >= ((Integer)cutOffMap.get(criteria)).intValue())) && ((!key.equalsIgnoreCase("Quality Graph")) || (marks != 0.0D)))
        {
          criteriaStr.add(marks < 10.0D ? new StringBuilder().append("0").append(marks).append(":").append(criteria).toString() : new StringBuilder().append(marks).append(":").append(criteria).toString());
        }
      }
      ...
      addData2DB(instName, enrollNo, key, Utility.getStr(criteriaStr, ((Integer)cutOffMap.get("TOP_SXN")).intValue()), Utility.getStr(profStr, 999));
    }
