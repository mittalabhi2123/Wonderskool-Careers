...
public class GenerateCharts {
	...
	private static CategoryDataset createDataset() throws Exception {
		...
		ResultSet dataSet = Utility.conn.createStatement().executeQuery(
				QueryCollection.STUDENT_RESULT.GET_EXISTING_DATA.replace("?", stuId.toLowerCase()));
		while (dataSet.next()) {
	...
	private static JFreeChart createChart(CategoryDataset categorydataset) {
		JFreeChart jfreechart = ChartFactory.createBarChart3D(chartTitle, xAxisTitle, "Score", categorydataset,
		...
		custombarrenderer3d.setBasePositiveItemLabelPosition(new ItemLabelPosition(ItemLabelAnchor.OUTSIDE12,
	...
	static class CustomBarRenderer3D extends BarRenderer3D {
		public Paint getItemPaint(int i, int j) {
			...
			double cutOff = 0.0D;
			try {
				String query = "SELECT param_value FROM system_variables WHERE  LOWER(param_name) = '?'".replace("?",
						categorydataset.getColumnKeys().get(j).toString().toLowerCase());
